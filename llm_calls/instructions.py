...
instructions_cap = """The single item up for review arrives as JSON in two parts - first the "student" block, then the item with its history and sentence plan - carrying:
...
EXAMPLES — one card of this type at each level, all for an unrelated card item, each developed independently for the genitive singular of the second declension, on δοῦλος. You write ONE card; these stand here only to show what changes with level and what does not.
beginner:
```json
{
...
card_rejection = """That card was checked word by word against the Greek New Testament and against the student's vocabulary, and sent back:
{findings}
Write a new card for the same item, to the same sentence plan. Return it in the same format."""
